...
void dump_temp_block(block_model_t *model, double *temp, char const *file)
{
	flp_t *flp = model->flp;
	int i;
	char str[STR_SIZE];
	...
	for (i=0; i < flp->n_units; i++) {
		if(strncmp(flp->units[i].name,"_eb",3)) {
			fprintf(fp, "%s\t%.1f\n", flp->units[i].name, temp[i]);
		}
	}

	/* This information is not important for our purposes
